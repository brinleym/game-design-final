...
private void AttemptMove(Vector3 end)
{
	...
	if (hit.collider == null) {
		...
		// create temp hitBox to prevent two players colliding, destroy after move
		...
		if (hit.collider != null && hit.collider.name.Contains(colorPower)) {
			...
			if (colorPower.Equals("Red", System.StringComparison.Ordinal)) {
				GameObject key = GameObject.FindGameObjectWithTag ("RKey");
				key.GetComponent<RedKeyUI>().SetHaveKey();
			} else if (colorPower.Equals("Green", System.StringComparison.Ordinal)) {
				GameObject key = GameObject.FindGameObjectWithTag ("GKey");
				key.GetComponent<GreenKeyUI>().SetHaveKey();
			} else {
				GameObject key = GameObject.FindGameObjectWithTag ("BKey");
				key.GetComponent<BlueKeyUI>().SetHaveKey();
			}
